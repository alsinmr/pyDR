...
import scipy
from pyDR.Kai import exchange_models as EM
from typing import Optional   #hmm I dont know if this is really necessary
...
class KaiMarkov():
    ...
    def create_data_container(self) -> None:
        """
    ...
    def is_res_in_dict(self, resid: int) -> str:
        """
    ...
    def add_dihedral_by_ids(self, atomlist: MDA.AtomGroup, name: Optional[str] = None) -> None:
        ...
        dih = {"atom_ids":[], "name":None, "id":None}
    ...
    def add_vector_for_ct_by_ids(self, atomlist : MDA.AtomGroup, name: str = "") -> None:
        if name in self.exclude:
            return
    ...
    def get_peptide_planes(self) -> None:
    ...
    def get_methyl_groups(self) -> None:
        """
    ...
    def select_simulation(self, number: int = None) -> None:
        """
    ...
    def save(self, dtype: str = "float16") -> None:
        """
    ...
    def calc(self, **kwargs) -> None:
        ...
        if dihedral_calc.sum() or ct_calc.sum():
            if exists(f"long_traj_vecs{self.length}.npy") and exists(f"long_traj_dihedrals{self.length}.npy"):
                ...
                self.dihedrals = np.load(f"long_traj_dihedrals{self.length}.npy").astype(self.default_float)
                self.ct_vectors = np.load(f"long_traj_vecs{self.length}.npy").astype(self.default_float)
            else:
                ...
                np.save(f"long_traj_vecs{self.length}.npy",self.ct_vectors.astype("float16"))
                np.save(f"long_traj_dihedrals{self.length}.npy", self.dihedrals.astype("float16"))
            if ct_calc.sum():
                if cuda_available():
                    count =ct_calc.sum()
                    chunk=1
    ...
    def construct_full_ct_from_states(self, markov_states: np.ndarray, avg_vecs: np.ndarray, axis: plt.axis, **kwargs) -> np.ndarray:
        """
        ...
        :return: nothing
        """
        # we need the batch dimension 1 for ct and vecs because the kernel is written to calculate multipe cts at once
        # one could rewrite the calc_CT_on_cuda function to handel this problem
        ...
        sparse = kwargs.get("sparse") if kwargs.get("sparse") else 10
        s2 = np.zeros(1)
        calc_CT_on_cuda(ct, s2, vecs, np.ones(1, dtype=bool), sparse=sparse)
        axis.semilogx(self.ct_x_axis, ct[0], color="green", linestyle="dotted")
        return ct[0]

    def compare_cts_with_detetors(self, *args, **kwargs) -> plt.axis:
        D = DR.data()
        cts = np.array([ct for ct in args])
        D.load(Ct={'Ct': cts,
        ...
        tick_labels = tick_labels[:n_dets - remove_S2]  # removed last label because no data were there
        page_2 = plt.figure()
        page_2.set_size_inches(8.3, 11.7 / 2)  # reduce page size for less cts
        ax0 = page_2.add_subplot(2, 1, 1)
        ...
        ax0.set_yticks([0, 1])
        ax = page_2.add_subplot(2, 1, 2)
        hatches = ["xx","oo","xxx","ooo","xxxx","oooo"]
        for d in range(n_dets - remove_S2):
            for r in range(cts.shape[0]):
                ax.bar(d+(.8/cts.shape[0])*r, fit.R[r, d], width=.8/cts.shape[0], color=plt.get_cmap("tab10")(d),
                       hatch=hatches[r], edgecolor="black")


        if kwargs.get("labels"):
            if len(kwargs.get("labels")) == cts.shape[0]:
                ax.legend(kwargs.get("labels"))
        page_2.suptitle(kwargs.get("title"))
        if remove_S2:
            maxR = np.max(fit.R[r, :-2])
        else:
            maxR = np.max(fit.R[r,:])
        ...
        ax.set_xticks(range(n_dets - remove_S2))
        ax.set_xticklabels(tick_labels, rotation=66)
        ax.set_ylabel(r'$\rho_n^{(\Theta,S)}$')
        page_2.tight_layout()
        return page_2

    def do_markov(self, num: int = 0, extend: bool = False) -> None:
        """
        ...
        for key in self.sim_dict["residues"].keys():
            if "ILE" in key or "LEU" in key:
                ...
                for vec in self.sim_dict["residues"][key]['ct_vecs']:
                    if extend and "met-to-plane_2" in vec['name']:
                        cc_vecs.append(vec['id'])
                    elif not extend and "C-C-2-plane" in vec['name']:
                        cc_vecs.append(vec['id'])
        ...
        if extend:
            ...
            ch3_2_states[:, :] += ((self.dihedrals[ch3_2, :] < 0) == (self.dihedrals[ch3_2, :] >= -120)).astype(
                'uint8') * 2



        # the following lines provide a little more functionality than they actually need here. you append the interested
        # states (like chi1, chi2, ch3...) and it willproduce a specific state number out of the possibilities
        # note, that it can only handle 3-site-hop states
        chain_states = []
        chain_states.append(chi1_states)
        chain_states.append(chi2_states)
        if extend:
            chain_states.append(ch3_2_states)

        for l in range(len(chain_states)):
            s = chain_states.pop()
            for n in [0,1,2]:
                states[s == n] += n*3**l
        # now every state value shuld have an index between 0 and 9 (or 27)
        ...
        n_states = 27 if extend else 9
        state_ticks = list(str(x) for x in range(1,n_states+1))
        for i in range(n_states):
            '''averaging all vectors in their specific state and store them for calculation of the P2 for C(t) later
            ...
            if not np.isnan(avg_vec[0]):
                ...
                a, b, g = vf_tools.getFrame(avg_vec, return_angles=1)
                betas.append(b)
                gammas.append(g)
        ...
        #  deleting empty rows/columns from the matrix
        for index in to_delete[::-1]:
            state_ticks.pop(index)
        ...
        plot_matrix = transition_matrix.copy()  # hard copy the matrix to set zero values to nan in order to have a
        plot_matrix[plot_matrix == 0] = np.nan     # 'cleaner' plot of the image
        ax[1].imshow(plot_matrix, vmin=0, vmax=1)
        ...
        # calculate the population for every state for the whole trajectory
        pop = []
        ...
        ct_from_t_matrix = np.zeros(self.length)
        calc_ct_from_transition_matrix(ct_from_t_matrix, transition_matrix, P2matrix, pop)

        ax[3].semilogx(self.ct_x_axis, self.cts[cc_vecs[num]], color="black")
        ct3 = self.construct_full_ct_from_states(states[num], avg_vecs_B, ax[3])
        ax[3].semilogx(self.ct_x_axis, ct_from_t_matrix, color="red")
        ...
        #todo something is still wrong with the EM handling
        exchange_matrix = scipy.linalg.logm(transition_matrix)
        try:
            ax[2].imshow(exchange_matrix.T)
        except TypeError as e:
            print("Exchange Matrix for {} could not be plotted because of:\n{}".format(res[num], e))

        # calculate equilibrium population (peq), S², tau values und A values
        peq, S2, tau, A = EM.ex2Ct(np.array(betas), np.array(gammas), exchange_matrix)
        # do the thing
        tau, A = EM.degen_tau(tau, A)
        Ct = np.ones(self.length)
        calc_ct_from_exchange_matrix(Ct, S2, A, tau)
        ax[3].semilogx(self.ct_x_axis, Ct, color="blue", linestyle="dashed")
        ax[3].legend(["original", "constructed", "markov", "exchangematrix"])
        ticks = np.array([10, 100, 1000, 10000, 100000, 1000000]).astype(int)
        ticks //= int(self.dt*(10**12))
        ax[3].set_ylim(0, 1.05)
        ax[3].set_xlim(self.ct_x_axis[np.array([0,-1])])
        ax[3].set_xticks(ticks)
        ax[3].set_yticks([0, .25, .5, .75, 1])
        ax[3].set_xticklabels(["10 ps", "100 ps", "1 ns", "10 ns", "100 ns", "1 µs"], rotation=30)

        fig2 = self.compare_cts_with_detetors(self.cts[cc_vecs[num]], ct3, ct_from_t_matrix, Ct,
                                              labels=["original", "constructed", "markov", "exchange"], title=res[num])
        ...
        for a in [ax[1],ax[2]]:
            a.set_xticks(range(len(state_ticks)))
            a.set_yticks(range(len(state_ticks)))
            a.set_xticklabels(state_ticks)
            a.set_yticklabels(state_ticks)
            a.set_xlabel("start state")
            a.set_ylabel("target state")

        fig.subplots_adjust(wspace=0.22, hspace=0.2, right=0.99, top=0.95, bottom=0.075, left=0.075)
        if not exists("markov/"):
            mkdir("markov/")
        fn = "markov/{}_C-C_bond.pdf" if not extend else "markov/{}_C-H_bond.pdf"
        with PdfPages(join(self.dir, fn.format(res[num]))) as pdf:
            pdf.savefig(fig)
            pdf.savefig(fig2)
            plt.close(fig)
            plt.close(fig2)
    ...
    def plot_single_res(self, label: str, plot_ct: bool = False) -> None:
        if plot_ct:
            fig = plt.figure()
    ...
    def plot_det_gui(self, residue: str):
